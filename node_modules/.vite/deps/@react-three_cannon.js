...
import { $r as MathUtils, Bo as SphereGeometry, Bs as Vector3, Do as Scene, K as BoxGeometry, Qn as InstancedMesh, Rt as CylinderGeometry, Vi as Object3D, Y as BufferGeometry, _a as Quaternion$2, ai as MeshBasicMaterial, hn as Euler, ia as PlaneGeometry, ii as Mesh, ni as Matrix4, on as DynamicDrawUsage, xn as Float32BufferAttribute } from "./three.module-C5Sn_aNr.js";
import { k as useThree, w as useFrame } from "./react-three-fiber.esm-CIu0IZxV.js";
import "./scheduler-Dp3MpxJh.js";
import { t as require_jsx_runtime } from "./react_jsx-runtime.js";
//#region node_modules/@react-three/cannon/dist/index.js
...
function decodeBase64(base64, enableUnicode) {
	...
	if (enableUnicode) {
		...
		for (var i = 0, n = binaryString.length; i < n; ++i) binaryView[i] = binaryString.charCodeAt(i);
...
function createURL(base64, sourcemapArg, enableUnicodeArg) {
	...
	var source = decodeBase64(base64, enableUnicodeArg === void 0 ? false : enableUnicodeArg);
...
var CannonWorkerAPI = class extends EventEmitter$1 {
	...
	worker = null;
	constructor(_ref) {
		let { allowSleep = false, axisIndex = 0, broadphase = "Naive", defaultContactMaterial = { contactEquationStiffness: 1e6 }, frictionGravity = null, gravity = [
			0,
			-9.81,
			0
		], iterations = 5, quatNormalizeFast = false, quatNormalizeSkip = 0, size = 1e3, solver = "GS", tolerance = .001 } = _ref;
		super();
		this.config = {
			allowSleep,
			axisIndex,
	...
	connect() {
		...
		for (const message of this.messageQueue) this.worker.postMessage(message);
	...
	step(props) {
		...
		(_this$worker = this.worker) == null || _this$worker.postMessage({
			op: "step",
			positions,
			props,
			quaternions
	...
	terminate() {
		var _this$worker2;
		(_this$worker2 = this.worker) == null || _this$worker2.terminate();
		this.worker = null;
	}
	unsubscribe(_ref53) {
		let { props } = _ref53;
...
var Mat3$1 = class Mat3$1 {
	...
	*/
	getTrace(target) {
		if (target === void 0) target = new Vec3$1();
		const e = this.elements;
		target.x = e[0];
		target.y = e[4];
		target.z = e[8];
	...
	*/
	smult(s) {
		for (let i = 0; i < this.elements.length; i++) this.elements[i] *= s;
	}
	/**
	...
	*/
	mmult(matrix, target) {
		if (target === void 0) target = new Mat3$1();
	...
	*/
	solve(b, target) {
		...
		do {
			i = k - n;
			if (eqns[i + nc * i] === 0) {
				for (j = i + 1; j < k; j++) if (eqns[i + nc * j] !== 0) {
					np = kp;
					do {
						p = kp - np;
			...
			if (eqns[i + nc * i] !== 0) for (j = i + 1; j < k; j++) {
		...
		if (isNaN(target.x) || isNaN(target.y) || isNaN(target.z) || target.x === Infinity || target.y === Infinity || target.z === Infinity) throw `Could not solve equation! Got x=[${target.toString()}], b=[${b.toString()}], A=[${this.toString()}]`;
		return target;
	}
	/**
	...
	*/
	e(row, column, value) {
		if (value === void 0) return this.elements[column + 3 * row];
		else this.elements[column + 3 * row] = value;
	}
	/**
	* Copy another matrix into this matrix object.
	*/
	copy(matrix) {
		for (let i = 0; i < matrix.elements.length; i++) this.elements[i] = matrix.elements[i];
		return this;
	}
	/**
	...
	*/
	toString() {
		let r = "";
		const sep = ",";
		for (let i = 0; i < 9; i++) r += this.elements[i] + sep;
		return r;
	}
	/**
	* reverse the matrix
	...
	*/
	reverse(target) {
		...
		do {
			...
			do {
				p = eqns[nr + j + nc * i];
				if (isNaN(p) || p === Infinity) throw `Could not reverse! A=[${this.toString()}]`;
				target.e(i, j, p);
			} while (j--);
		} while (i--);
		return target;
...
var Vec3$1 = class Vec3$1 {
	...
	*/
	vadd(vector, target) {
		...
		} else return new Vec3$1(this.x + vector.x, this.y + vector.y, this.z + vector.z);
	}
	/**
	* Vector subtraction
	* @param target Optional target to save in.
	...
	*/
	almostEquals(vector, precision) {
		...
		if (Math.abs(this.x - vector.x) > precision || Math.abs(this.y - vector.y) > precision || Math.abs(this.z - vector.z) > precision) return false;
		return true;
	}
	/**
...
var AABB$1 = class AABB$1 {
	...
	*/
	constructor(options) {
		...
		if (options.lowerBound) this.lowerBound.copy(options.lowerBound);
		if (options.upperBound) this.upperBound.copy(options.upperBound);
	}
	/**
...
var transformIntoFrame_corners$1 = [
	new Vec3$1(),
	new Vec3$1(),
	new Vec3$1(),
	new Vec3$1(),
...
var EventTarget = class {
	...
	*/
	hasEventListener(type, listener) {
		...
		const listeners = this._listeners;
		if (listeners[type] !== void 0 && listeners[type].includes(listener)) return true;
		return false;
	}
	/**
	...
	*/
	hasAnyEventListener(type) {
		if (this._listeners === void 0) return false;
		return this._listeners[type] !== void 0;
	}
	/**
	...
	*/
	removeEventListener(type, listener) {
		...
		const listeners = this._listeners;
		if (listeners[type] === void 0) return this;
...
*/
var Quaternion$1 = class Quaternion$1 {
	constructor(x, y, z, w) {
		if (x === void 0) x = 0;
		if (y === void 0) y = 0;
		if (z === void 0) z = 0;
	...
	*/
	toAxisAngle(targetAxis) {
		if (targetAxis === void 0) targetAxis = new Vec3$1();
		this.normalize();
	...
	*/
	mult(quat, target) {
		if (target === void 0) target = new Quaternion$1();
		const ax = this.x;
		const ay = this.y;
		const az = this.z;
		const aw = this.w;
	...
	toEuler(target, order) {
		...
		switch (order) {
			case "YZX":
				...
				break;
			default: throw new Error(`Euler order ${order} not supported yet.`);
		}
		target.y = heading;
		target.z = attitude;
		target.x = bank;
...
*/
var Transform$1 = class Transform$1 {
	/**
	* position
	*/
	/**
	...
	*/
	vectorToWorldFrame(localVector, result) {
		if (result === void 0) result = new Vec3$1();
...
*/
var ConvexPolyhedron = class ConvexPolyhedron extends Shape$1 {
	/** vertices */
	/**
	...
	*/
	constructor(props) {
		...
		super({ type: Shape$1.types.CONVEXPOLYHEDRON });
		this.vertices = vertices;
		this.faces = faces;
		this.faceNormals = normals;
		if (this.faceNormals.length === 0) this.computeNormals();
		if (!boundingSphereRadius) this.updateBoundingSphereRadius();
		else this.boundingSphereRadius = boundingSphereRadius;
		this.worldVertices = [];
	...
	computeEdges() {
		...
		for (let i = 0; i !== faces.length; i++) {
			...
			for (let j = 0; j !== numVertices; j++) {
				...
				edge.normalize();
				let found = false;
				for (let p = 0; p !== edges.length; p++) if (edges[p].almostEquals(edge) || edges[p].almostEquals(edge)) {
					found = true;
					break;
				}
	...
	*/
	computeNormals() {
		...
		for (let i = 0; i < this.faces.length; i++) {
			for (let j = 0; j < this.faces[i].length; j++) if (!this.vertices[this.faces[i][j]]) throw new Error(`Vertex ${this.faces[i][j]} not found!`);
			...
			if (n.dot(vertex) < 0) {
				...
				for (let j = 0; j < this.faces[i].length; j++) console.warn(`.vertices[${this.faces[i][j]}] = Vec3(${this.vertices[this.faces[i][j]].toString()})`);
			}
		}
	}
	/**
	...
	*/
	static computeNormal(va, vb, vc, target) {
		...
		vc.vsub(vb, cb);
		cb.cross(ab, target);
		if (!target.isZero()) target.normalize();
	}
	/**
	...
	*/
	findSeparatingAxis(hullB, posA, quatA, posB, quatB, target, faceListA, faceListB) {
		...
		} else for (let i = 0; i !== hullA.uniqueAxes.length; i++) {
		...
		} else for (let i = 0; i !== hullB.uniqueAxes.length; i++) {
		...
		if (deltaC.dot(target) > 0) target.negate(target);
		return true;
	}
	/**
	...
	*/
	clipFaceAgainstHull(separatingNormal, posA, quatA, worldVertsB1, minDist, maxDist, result) {
		...
		for (let i = 0; i < hullA.faces.length; i++) for (let j = 0; j < hullA.faces[i].length; j++) if (polyA.indexOf(hullA.faces[i][j]) !== -1 && i !== closestFaceA && polyA.connectedFaces.indexOf(i) === -1) polyA.connectedFaces.push(i);
	...
	*/
	computeWorldVertices(position, quat) {
		while (this.worldVertices.length < this.vertices.length) this.worldVertices.push(new Vec3$1());
	...
	*/
	computeWorldFaceNormals(quat) {
		...
		while (this.worldFaceNormals.length < N) this.worldFaceNormals.push(new Vec3$1());
		const normals = this.faceNormals;
		const worldNormals = this.worldFaceNormals;
		for (let i = 0; i !== N; i++) quat.vmult(normals[i], worldNormals[i]);
	...
	*/
	calculateWorldAABB(pos, quat, min, max) {
		...
		for (let i = 0; i < verts.length; i++) {
			...
			if (minx === void 0 || v.x < minx) minx = v.x;
			if (maxx === void 0 || v.x > maxx) maxx = v.x;
			if (miny === void 0 || v.y < miny) miny = v.y;
			if (maxy === void 0 || v.y > maxy) maxy = v.y;
			if (minz === void 0 || v.z < minz) minz = v.z;
			if (maxz === void 0 || v.z > maxz) maxz = v.z;
		}
	...
	*/
	getAveragePointLocal(target) {
		if (target === void 0) target = new Vec3$1();
		const verts = this.vertices;
		for (let i = 0; i < verts.length; i++) target.vadd(verts[i], target);
...
*/
var Box = class Box extends Shape$1 {
	/**
	...
	*/
	getSideNormals(sixTargetVectors, quat) {
		...
		if (quat !== void 0) for (let i = 0; i !== sides.length; i++) quat.vmult(sides[i], sides[i]);
		return sides;
	}
	/**
...
*     world.addBody(body)
*/
var Body = class Body extends EventTarget {
	/**
	...
	*/
	constructor(options) {
		...
		if (options.linearFactor) this.linearFactor.copy(options.linearFactor);
		this.angularFactor = new Vec3$1(1, 1, 1);
		if (options.angularFactor) this.angularFactor.copy(options.angularFactor);
	...
	*/
	wakeUp() {
		...
		this.sleepState = Body.AWAKE;
		this.wakeUpAfterNarrowphase = false;
		if (prevState === Body.SLEEPING) this.dispatchEvent(Body.wakeupEvent);
	}
	/**
	* Force body sleep
	*/
	...
	*/
	sleepTick(time) {
		if (this.allowSleep) {
			...
			} else if (sleepState === Body.SLEEPY && speedSquared > speedLimitSquared) this.wakeUp();
	...
	*/
	addShape(shape, _offset, _orientation) {
		...
		if (_offset) offset.copy(_offset);
		if (_orientation) orientation.copy(_orientation);
	...
	*/
	updateAABB() {
		...
		for (let i = 0; i !== N; i++) {
			...
			if (i === 0) aabb.copy(shapeAABB);
			else aabb.extend(shapeAABB);
		}
	...
	*/
	applyForce(force, relativePoint) {
		if (relativePoint === void 0) relativePoint = new Vec3$1();
		if (this.type !== Body.DYNAMIC) return;
		if (this.sleepState === Body.SLEEPING) this.wakeUp();
	...
	*/
	applyLocalForce(localForce, localPoint) {
		if (localPoint === void 0) localPoint = new Vec3$1();
		if (this.type !== Body.DYNAMIC) return;
	...
	*/
	integrate(dt, quatNormalize, quatNormalizeFast) {
		...
		if (quatNormalize) if (quatNormalizeFast) quat.normalizeFast();
		else quat.normalize();
		this.aabbNeedsUpdate = true;
...
var Ray$1 = class Ray$1 {
	...
	*/
	...
	constructor(from, to) {
		if (from === void 0) from = new Vec3$1();
		if (to === void 0) to = new Vec3$1();
	...
	*/
	intersectBodies(bodies, result) {
		...
		for (let i = 0, l = bodies.length; !this.result.shouldStop && i < l; i++) this.intersectBody(bodies[i]);
	}
	/**
	...
	*/
	...
	intersectShape(shape, quat, position, body) {
		...
		if (distanceFromIntersection$1(from, this.direction, position) > shape.boundingSphereRadius) return;
	...
	_intersectPlane(shape, quat, position, body, reportedShape) {
		...
		if (planeToFrom * len.dot(worldNormal) > 0) return;
	...
	*/
	...
	_intersectHeightfield(shape, quat, position, body, reportedShape) {
		...
		for (let i = iMinX; i < iMaxX; i++) for (let j = iMinY; j < iMaxY; j++) {
	...
	_intersectConvex(shape, quat, position, body, reportedShape, options) {
		...
		for (let j = 0; !result.shouldStop && j < Nfaces; j++) {
			...
			for (let i = 1; !result.shouldStop && i < face.length - 1; i++) {
				...
				if (!(Ray$1.pointInTriangle(intersectPoint$1, a$1, b$1, c$1) || Ray$1.pointInTriangle(intersectPoint$1, b$1, a$1, c$1)) || distance > fromToDistance) continue;
	...
	*/
	_intersectTrimesh(mesh, quat, position, body, reportedShape, options) {
		...
		for (let i = 0, N = triangles.length; !this.result.shouldStop && i !== N; i++) {
			...
			if (!(Ray$1.pointInTriangle(intersectPoint$1, b$1, a$1, c$1) || Ray$1.pointInTriangle(intersectPoint$1, a$1, b$1, c$1)) || squaredDistance > fromToDistanceSquared) continue;
	...
	*/
	reportIntersection(normal, hitPointWorld, shape, body, hitFaceIndex) {
		...
		const result = this.result;
		if (this.skipBackfaces && normal.dot(this.direction) > 0) return;
...
new Vec3$1();
new RaycastResult$1();
var intersectConvexOptions$1 = { faceList: [0] };
...
function distanceFromIntersection$1(from, direction, position) {
	...
	return position.distanceTo(intersect$1);
}
var Utils = class {
	/**
	...
	*/
	static defaults(options, defaults) {
		if (options === void 0) options = {};
		for (let key in defaults) if (!(key in options)) options[key] = defaults[key];
		return options;
	}
};
new Vec3$1();
...
var Sphere = class extends Shape$1 {
	...
	*/
	constructor(radius) {
		super({ type: Shape$1.types.SPHERE });
...
var Cylinder = class extends ConvexPolyhedron {
	...
	*/
	constructor(radiusTop, radiusBottom, height, numSegments) {
		...
		if (numSegments === void 0) numSegments = 8;
		if (radiusTop < 0) throw new Error("The cylinder radiusTop cannot be negative.");
		if (radiusBottom < 0) throw new Error("The cylinder radiusBottom cannot be negative.");
		const N = numSegments;
		const vertices = [];
		const axes = [];
		const faces = [];
		...
		const cos = Math.cos;
		const sin = Math.sin;
		vertices.push(new Vec3$1(-radiusBottom * sin(0), -height * .5, radiusBottom * cos(0)));
		bottomface.push(0);
		vertices.push(new Vec3$1(-radiusTop * sin(0), height * .5, radiusTop * cos(0)));
		topface.push(1);
		for (let i = 0; i < N; i++) {
			const theta = 2 * Math.PI / N * (i + 1);
			const thetaN = 2 * Math.PI / N * (i + .5);
			if (i < N - 1) {
				vertices.push(new Vec3$1(-radiusBottom * sin(theta), -height * .5, radiusBottom * cos(theta)));
				bottomface.push(2 * i + 2);
				vertices.push(new Vec3$1(-radiusTop * sin(theta), height * .5, radiusTop * cos(theta)));
				topface.push(2 * i + 3);
				faces.push([
					2 * i,
					2 * i + 1,
			...
			if (N % 2 === 1 || i < N / 2) axes.push(new Vec3$1(-sin(thetaN), 0, cos(thetaN)));
		}
		faces.push(bottomface);
		axes.push(new Vec3$1(0, 1, 0));
		const temp = [];
		for (let i = 0; i < topface.length; i++) temp.push(topface[topface.length - i - 1]);
		faces.push(temp);
		super({
			vertices,
			faces,
...
var Particle = class extends Shape$1 {
	constructor() {
		super({ type: Shape$1.types.PARTICLE });
	}
	/**
	* calculateLocalInertia
	*/
...
var Plane = class extends Shape$1 {
	...
	constructor() {
		super({ type: Shape$1.types.PLANE });
...
var Heightfield = class extends Shape$1 {
	...
	*/
	constructor(data, options) {
		...
		super({ type: Shape$1.types.HEIGHTFIELD });
		this.data = data;
		this.maxValue = options.maxValue;
		this.minValue = options.minValue;
		this.elementSize = options.elementSize;
		if (options.minValue === null) this.updateMinValue();
		if (options.maxValue === null) this.updateMaxValue();
	...
	*/
	setHeightValueAtIndex(xi, yi, value) {
		...
		if (yi > 0 && xi > 0) this.clearCachedConvexTrianglePillar(xi - 1, yi - 1, true);
	}
	/**
	...
	*/
	...
	getTriangleAt(x, y, edgeClamp, a, b, c) {
		...
		const upper = (x / elementSize - xi) ** 2 + (y / elementSize - yi) ** 2 > (x / elementSize - (xi + 1)) ** 2 + (y / elementSize - (yi + 1)) ** 2;
	...
	*/
	getHeightAt(x, y, edgeClamp) {
		...
		if (upper) return data[xi + 1][yi + 1] * w.x + data[xi][yi + 1] * w.y + data[xi + 1][yi] * w.z;
		else return data[xi][yi] * w.x + data[xi + 1][yi] * w.y + data[xi][yi + 1] * w.z;
	}
	...
	*/
	getConvexTrianglePillar(xi, yi, getUpperTriangle) {
		...
		for (let i = 0; i < 6; i++) if (!result.vertices[i]) result.vertices[i] = new Vec3$1();
		faces.length = 5;
		for (let i = 0; i < 5; i++) if (!faces[i]) faces[i] = [];
		...
		if (!getUpperTriangle) {
			...
			verts[3].set(-.25 * elementSize, -.25 * elementSize, -Math.abs(h) - 1);
			verts[4].set(.75 * elementSize, -.25 * elementSize, -Math.abs(h) - 1);
			verts[5].set(-.25 * elementSize, .75 * elementSize, -Math.abs(h) - 1);
			faces[0][0] = 0;
			faces[0][1] = 1;
			faces[0][2] = 2;
			faces[1][0] = 5;
	...
	setHeightsFromImage(image, scale) {
		...
		for (let i = 0; i < imageData.height; i++) {
			...
			for (let j = 0; j < imageData.width; j++) {
				...
				if (x < 0) row.push(height);
				else row.unshift(height);
			}
			if (y < 0) matrix.unshift(row);
			else matrix.push(row);
		}
		this.updateMaxValue();
		this.updateMinValue();
		this.update();
...
* OctreeNode
*/
var OctreeNode = class OctreeNode {
	/** The root node */
	...
	*/
	insert(aabb, elementData, level) {
		...
		if (level < maxDepth) {
			...
			for (let i = 0; i !== 8; i++) if (children[i].insert(aabb, elementData, level + 1)) return true;
	...
	*/
	subdivide() {
		...
		const children = this.children;
		children.push(new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(0, 0, 0) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(1, 0, 0) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(1, 1, 0) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(1, 1, 1) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(0, 1, 1) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(0, 0, 1) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(1, 0, 1) }) }), new OctreeNode({ aabb: new AABB$1({ lowerBound: new Vec3$1(0, 1, 0) }) }));
	...
	*/
	aabbQuery(aabb, result) {
		...
		while (queue.length) {
			...
			if (node.aabb.overlaps(aabb)) Array.prototype.push.apply(result, node.data);
	...
	*/
	removeEmptyNodes() {
		for (let i = this.children.length - 1; i >= 0; i--) {
			...
			if (!this.children[i].children.length && !this.children[i].data.length) this.children.splice(i, 1);
		}
	}
};
/**
...
*/
var Trimesh = class Trimesh extends Shape$1 {
	/**
	* vertices
	*/
	/**
	...
	*/
	constructor(vertices, indices) {
		super({ type: Shape$1.types.TRIMESH });
	...
	*/
	setScale(scale) {
		...
		if (!(wasUniform && isUniform)) this.updateNormals();
	...
	*/
	computeLocalAABB(aabb) {
		...
		for (let i = 0; i !== n; i++) {
			this.getVertex(i, v);
			if (v.x < l.x) l.x = v.x;
			else if (v.x > u.x) u.x = v.x;
			if (v.y < l.y) l.y = v.y;
			else if (v.y > u.y) u.y = v.y;
			if (v.z < l.z) l.z = v.z;
			else if (v.z > u.z) u.z = v.z;
		}
	}
	/**
...
Shape$1.types.SPHERE, Shape$1.types.SPHERE | Shape$1.types.PLANE, Shape$1.types.BOX | Shape$1.types.BOX, Shape$1.types.SPHERE | Shape$1.types.BOX, Shape$1.types.PLANE | Shape$1.types.BOX, Shape$1.types.CONVEXPOLYHEDRON, Shape$1.types.SPHERE | Shape$1.types.CONVEXPOLYHEDRON, Shape$1.types.PLANE | Shape$1.types.CONVEXPOLYHEDRON, Shape$1.types.BOX | Shape$1.types.CONVEXPOLYHEDRON, Shape$1.types.SPHERE | Shape$1.types.HEIGHTFIELD, Shape$1.types.BOX | Shape$1.types.HEIGHTFIELD, Shape$1.types.CONVEXPOLYHEDRON | Shape$1.types.HEIGHTFIELD, Shape$1.types.PARTICLE | Shape$1.types.SPHERE, Shape$1.types.PLANE | Shape$1.types.PARTICLE, Shape$1.types.BOX | Shape$1.types.PARTICLE, Shape$1.types.PARTICLE | Shape$1.types.CONVEXPOLYHEDRON, Shape$1.types.CYLINDER, Shape$1.types.SPHERE | Shape$1.types.CYLINDER, Shape$1.types.PLANE | Shape$1.types.CYLINDER, Shape$1.types.BOX | Shape$1.types.CYLINDER, Shape$1.types.CONVEXPOLYHEDRON | Shape$1.types.CYLINDER, Shape$1.types.HEIGHTFIELD | Shape$1.types.CYLINDER, Shape$1.types.PARTICLE | Shape$1.types.CYLINDER, Shape$1.types.SPHERE | Shape$1.types.TRIMESH, Shape$1.types.PLANE | Shape$1.types.TRIMESH;
new Vec3$1();
new Vec3$1();
new Vec3$1();
new Vec3$1();
...
if (!performance$1.now) {
	...
	if (performance$1.timing && performance$1.timing.navigationStart) nowOffset = performance$1.timing.navigationStart;
...
var prepareConvexPolyhedron = (_ref2) => {
	...
	return [{
		axes: a ? a.map(makeVec3) : void 0,
		boundingSphereRadius,
		faces,
		normals: n ? n.map(makeVec3) : void 0,
		vertices: v ? v.map(makeVec3) : void 0
	}];
};
function createShape(type, args) {
	switch (type) {
		case "Box": return new Box(new Vec3$1(...args.map((v) => v / 2)));
		case "ConvexPolyhedron": return new ConvexPolyhedron(...prepareConvexPolyhedron(args));
		case "Cylinder": return new Cylinder(...args);
		case "Heightfield": return new Heightfield(...args);
		case "Particle": return new Particle();
		case "Plane": return new Plane();
		case "Sphere": return new Sphere(...prepareSphere(args));
		case "Trimesh": return new Trimesh(...args);
	}
}
/**
...
var setQuaternion = (target, _ref3) => {
	...
	if (quaternion) target.set(...quaternion);
	else if (rotation) target.setFromEuler(...rotation);
	return target;
};
/**
* @function
...
var propsToBody = (options) => {
	...
	body.uuid = uuid;
	if (collisionResponse !== void 0) body.collisionResponse = collisionResponse;
	if (type === "Compound") shapes.forEach((_ref4) => {
		let { type, args, position, rotation, quaternion, material, ...extra } = _ref4;
		const shapeBody = body.addShape(createShape(type, args), position ? new Vec3$1(...position) : void 0, setQuaternion(new Quaternion$1(0, 0, 0, 1), {
			quaternion,
			rotation
		}));
	...
	else body.addShape(createShape(type, args));
	...
	*/
	vadd(vector, target) {
		if (target) {
			...
			target.y = vector.y + this.y;
			target.z = vector.z + this.z;
		} else return new Vec3(this.x + vector.x, this.y + vector.y, this.z + vector.z);
	}
	/**
	* Vector subtraction
	* @param target Optional target to save in.
...
var Quaternion = class Quaternion {
	...
	*/
	toAxisAngle(targetAxis) {
		if (targetAxis === void 0) targetAxis = new Vec3();
		this.normalize();
		const angle = 2 * Math.acos(this.w);
		const s = Math.sqrt(1 - this.w * this.w);
		if (s < .001) {
...
var Ray = class Ray {
	...
	*/
	...
	intersectShape(shape, quat, position, body) {
		const from = this.from;
		if (distanceFromIntersection(from, this.direction, position) > shape.boundingSphereRadius) return;
		const intersectMethod = this[shape.type];
		if (intersectMethod) intersectMethod.call(this, shape, quat, position, body, shape);
	}
	_intersectBox(box, quat, position, body, reportedShape) {
	...
	_intersectConvex(shape, quat, position, body, reportedShape, options) {
		...
		for (let j = 0; !result.shouldStop && j < Nfaces; j++) {
			...
			for (let i = 1; !result.shouldStop && i < face.length - 1; i++) {
				...
				if (!(Ray.pointInTriangle(intersectPoint, a, b, c) || Ray.pointInTriangle(intersectPoint, b, a, c)) || distance > fromToDistance) continue;
	...
	*/
	_intersectTrimesh(mesh, quat, position, body, reportedShape, options) {
		...
		for (let i = 0, N = triangles.length; !this.result.shouldStop && i !== N; i++) {
			...
			if (!(Ray.pointInTriangle(intersectPoint, b, a, c) || Ray.pointInTriangle(intersectPoint, a, b, c)) || squaredDistance > fromToDistanceSquared) continue;
...
new Transform();
new AABB();
Shape.types.SPHERE, Shape.types.SPHERE | Shape.types.PLANE, Shape.types.BOX | Shape.types.BOX, Shape.types.SPHERE | Shape.types.BOX, Shape.types.PLANE | Shape.types.BOX, Shape.types.CONVEXPOLYHEDRON, Shape.types.SPHERE | Shape.types.CONVEXPOLYHEDRON, Shape.types.PLANE | Shape.types.CONVEXPOLYHEDRON, Shape.types.BOX | Shape.types.CONVEXPOLYHEDRON, Shape.types.SPHERE | Shape.types.HEIGHTFIELD, Shape.types.BOX | Shape.types.HEIGHTFIELD, Shape.types.CONVEXPOLYHEDRON | Shape.types.HEIGHTFIELD, Shape.types.PARTICLE | Shape.types.SPHERE, Shape.types.PLANE | Shape.types.PARTICLE, Shape.types.BOX | Shape.types.PARTICLE, Shape.types.PARTICLE | Shape.types.CONVEXPOLYHEDRON, Shape.types.CYLINDER, Shape.types.SPHERE | Shape.types.CYLINDER, Shape.types.PLANE | Shape.types.CYLINDER, Shape.types.BOX | Shape.types.CYLINDER, Shape.types.CONVEXPOLYHEDRON | Shape.types.CYLINDER, Shape.types.HEIGHTFIELD | Shape.types.CYLINDER, Shape.types.PARTICLE | Shape.types.CYLINDER, Shape.types.SPHERE | Shape.types.TRIMESH, Shape.types.PLANE | Shape.types.TRIMESH;
new Vec3();
new Vec3();
new Vec3();
new Vec3();
...
function CannonDebugger(scene, world, _temp) {
	let { color = 65280, scale = 1, onInit, onUpdate } = _temp === void 0 ? {} : _temp;
	const _meshes = [];
...
var debugContext = /* @__PURE__ */ (0, import_react.createContext)(null);
var useDebugContext = () => (0, import_react.useContext)(debugContext);
var physicsContext = /* @__PURE__ */ (0, import_react.createContext)(null);
...
function DebugProvider(_ref) {
	...
	const [{ bodies, bodyMap }] = (0, import_react.useState)({
		bodies: [],
		bodyMap: {}
	});
	const { refs } = usePhysicsContext();
	const [scene] = (0, import_react.useState)(() => new Scene());
	const cannonDebuggerRef = (0, import_react.useRef)(impl(scene, { bodies }, {
		color,
		scale
	}));
	useFrame(() => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(debugContext.Provider, {
		value: api,
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("primitive", { object: scene }), children]
	});
}
...
function subscribe(ref, worker, subscriptions, type, index, target) {
	if (target === void 0) target = "bodies";
	return (callback) => {
...
function useBody(type, fn, argsFn, fwdRef, deps) {
	...
	(0, import_react.useLayoutEffect)(() => {
		if (!ref.current) ref.current = new Object3D();
	...
	return [ref, (0, import_react.useMemo)(() => {
		...
		const makeQuaternion = (index) => {
			return {
				...
				subscribe: subscribe(ref, worker, subscriptions, "quaternion", index)
			};
		};
...
function useBox(fn, fwdRef, deps) {
	...
	return useBody("Box", fn, function(args) {
		if (args === void 0) args = defaultBoxArgs;
		return args;
	}, fwdRef, deps);
}
...
function usePointToPointConstraint(bodyA, bodyB, optns, deps) {
	if (bodyA === void 0) bodyA = null;
	if (bodyB === void 0) bodyB = null;
	if (deps === void 0) deps = [];
...
function useRaycastVehicle(fn, fwdRef, deps) {
	...
	return [ref, (0, import_react.useMemo)(() => {
		return {
			...
			sliding: { subscribe: subscribe(ref, worker, subscriptions, "sliding", void 0, "vehicles") }
		};
	}, deps)];
}
...
function PhysicsProvider(_ref) {
	let { allowSleep = false, axisIndex = 0, broadphase = "Naive", children, defaultContactMaterial = { contactEquationStiffness: 1e6 }, frictionGravity = null, gravity = [
		0,
		-9.81,
		0
	], isPaused = false, iterations = 5, maxSubSteps = 10, quatNormalizeFast = false, quatNormalizeSkip = 0, shouldInvalidate = true, size = 1e3, solver = "GS", stepSize = 1 / 60, tolerance = .001 } = _ref;
	const { invalidate } = useThree();
	const [{ bodies, events, refs, scaleOverrides, subscriptions, worker }] = (0, import_react.useState)(() => ({
		bodies: {},
		events: {},
		refs: {},
		scaleOverrides: {},
	...
	const frameHandler = (_ref5) => {
		...
		for (let i = 0; i < uuids.length; i++) bodies[uuids[i]] = i;
		observations.forEach((_ref6) => {
			let [id, value, type] = _ref6;
			const cb = (subscriptions[id] || {})[type];
			cb && cb(value);
		});
		if (!active) return;
		for (const ref of Object.values(refs).filter(unique())) if (ref instanceof InstancedMesh) for (let i = 0; i < ref.count; i++) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(physicsContext.Provider, {
		value,
		children
	});
}
